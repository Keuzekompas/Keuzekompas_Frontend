...
const AiSupportPage = () => {
  ...
  const [availableTags, setAvailableTags] = useState<string[]>(CATEGORY_KEYS[language === 'NL' ? 'NL' : 'EN']);
  ...
  useEffect(() => {
      ...
      setTags(prevTags => {
          ...
          return prevTags.map(tag => {
              const idx = oldList.indexOf(tag);
              return idx !== -1 ? newList[idx] : tag;
      ...
      <Card className="w-full max-w-lg bg-(--bg-card) mt-8 transition-all duration-300">
        ...
        <CardContent>
          <div className="flex flex-col w-full">
            <label htmlFor="interests" className="mb-2 text-(--text-primary) font-medium">{t('ai.interestsLabel')}</label>
            <textarea
              id="interests"
              className={`p-3 border rounded-lg outline-none bg-(--bg-input) text-(--text-primary) h-48 resize-none transition-colors ${
                  interestsError ? 'border-(--color-error)' : 'border-(--border-input) focus:border-(--color-brand)'
              }`}
              value={interests}
              onChange={(e) => {
            ...
            <div className="min-h-6 mt-1 mb-2">
                {interestsError && <p className="text-(--color-error) text-sm animate-in fade-in slide-in-from-top-1">{interestsError}</p>}
            </div>

            <div className="flex justify-between gap-4 mb-1">
              <div className="w-1/2">
                <label htmlFor="location" className="block text-(--text-primary) font-medium min-h-12 items-end pb-2">{t('ai.locationLabel')}</label>
                <div className="relative">
                    ...
                    >
                    <option value="Geen">{t('ai.options.none-location')}</option>
              ...
              <div className="w-1/2">
                <label htmlFor="ecs" className="block text-(--text-primary) font-medium min-h-12 items-end pb-2">{t('ai.ectsLabel')}</label>
                 <div className="relative">
                    <select
                    id="ecs"
             ...
             <div className="min-h-6 mb-2">
                {ecsError && <p className="text-(--color-error) text-sm animate-in fade-in slide-in-from-top-1">{ecsError}</p>}
             </div>

            <label className="mb-2 text-(--text-primary) font-medium">{t('ai.tagsLabel')} (Min 1, Max 3)</label>
            <div className={`grid grid-cols-2 gap-2 mb-2 p-2 rounded-lg border ${tagsError ? 'border-(--color-error)' : 'border-transparent'}`}>
              {availableTags.map(tag => (
                <button
                  ...
                  } ${!tags.includes(tag) && tags.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
             ...
             <div className="min-h-6 mb-2">
                {tagsError && <p className="text-(--color-error) text-sm animate-in fade-in slide-in-from-top-1">{tagsError}</p>}
            </div>
